...
func (f *FileBasedQueue[T]) readAndRemove() (T, error) {
	...
	bdata, filename, err := ioutl.OldestFile(f.directory, preffix, suffix)
	if err != nil {
		var d T
	...
	if err = decoder.Decode(&data); err != nil {
		var d T
		ioutl.Rename(*filename, *filename+".error")
		return d, errors.Join(errors.New("error decoding"), err)
	}
	ioutl.Remove(*filename)
	return data, nil
}
...
func (f *FileBasedQueue[T]) writeData(data T) error {
	...
	err = ioutl.WriteToRandomFile(f.directory, preffix, suffix, buffer.Bytes())
	if err != nil {
		return err
	}
	return nil
...
func queueDirectory(directory string, id string) string {
	return ioutl.BuildFullPath([]string{directory, id})
}
